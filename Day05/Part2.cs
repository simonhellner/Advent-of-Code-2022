...
namespace Day05
{
    public static class Part2
    {
        ...
        {
            var toBeMoved = stacks[moveInstruction.From].TakeLast(moveInstruction.MoveCount);
            stacks[moveInstruction.To] += new string(toBeMoved.ToArray());
            var from = stacks[moveInstruction.From].ToList();
            from.RemoveRange(from.Count - moveInstruction.MoveCount, moveInstruction.MoveCount);
            stacks[moveInstruction.From] = new string(from.ToArray());
